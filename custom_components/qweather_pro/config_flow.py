...
# cryptography 仅在 JWT 模式下需要，使用延迟导入避免 API Key 用户因依赖问题卡死
try:
...
class QWeatherConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    def _generate_key_pair_sync(self) -> tuple[str, str]:
        """同步生成 JWT 密钥对."""
        if not HAS_CRYPTO:
            raise RuntimeError("cryptography 库未安装，无法生成 JWT 密钥对")
                ...
                if not HAS_CRYPTO:
                    return self.async_show_form(
                        ...
                        data_schema=vol.Schema({
                            ...
                            vol.Required(CONF_LOCATION_ID, default=self._temp_data.get(CONF_LOCATION_ID, "")): selector.TextSelector(),
